...
1. **Causal Fidelity & Amortization (PGExplainer)**: PGExplainer achieves the highest **Necessity score (0.166)**, demonstrating that it is highly effective at identifying edge subgraphs that are causally essential for the GAT's predictions. Because PGExplainer trains a parameterized MLP globally across instances, it learns generalized topological motifs that govern DDI predictions. However, this global regularization can occasionally miss case-specific drug-pair pathways, leading to a slightly lower **Sufficiency score (0.558)**.
2. **Topological Connectivity & Counterfactuals (KEC)**: The proposed Knowledge-Enhanced Counterfactual (KEC) method achieves a competitive **Necessity score (0.122)** and the highest **Sufficiency score (0.575)**. KEC explicitly searches for the minimal causal set of topological links to flip the prediction label under counterfactual conditions, ensuring that keeping only the KEC edges preserves the prediction state well.
3. **Failure of GNNExplainer and Attention on Necessity (Descriptive vs. Causal)**: Both Attention Rollout (0.002) and GNNExplainer (0.000) show near-zero Necessity. This indicates that removing their selected explanation subgraphs does not impact predictions because alternative redundant message flows remain active in the graph. Thus, Attention and vanilla GNNExplainer mainly provide descriptive (correlated) rather than causal explanations.

### 5.6.1 Statistical Significance & Connectivity Analysis
To evaluate the statistical significance of these results, we performed a Wilcoxon signed-rank test across the 100 evaluated drug pairs. The difference in necessity between PGExplainer (0.166) and KEC (0.122) is not statistically significant ($p > 0.05$), indicating both methods identify highly necessary edge subgraphs. However, both significantly outperform Attention Rollout and GNNExplainer on necessity ($p < 0.001$), confirming that local counterfactual constraints and parameterized models yield much stronger causal relevance.
...
These findings highlight that combining parameterized global models (PGExplainer) and biologically-grounded counterfactual constraints (KEC) is essential for identifying necessary, sufficient, and interpretable clinical pathways in heterogeneous networks.
