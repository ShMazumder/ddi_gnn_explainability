...
# Find step 3 code cell (contains build_kg = importlib.import_module("scripts.02_build_kg"))
cell_idx = None
for idx, cell in enumerate(nb["cells"]):
    if cell["cell_type"] == "code" and any("build_kg = importlib.import_module" in line for line in cell["source"]):
        cell_idx = idx
        break
...
if cell_idx is not None:
    ...
    new_source = [
        ...
        "if \"..\" not in sys.path:\n",
        "    sys.path.append(\"..\")\n",
        "import scripts.02_build_kg\n",
        "importlib.reload(scripts.02_build_kg)\n",
        "\n",
        "# Build the graph streamingly using the optimized script\n",
        "scripts.02_build_kg.build_graph()\n",
        "\n",
    ...
    print("Notebook Step 3 updated successfully with reload!")
else:
